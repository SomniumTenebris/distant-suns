    ...
    def update(self, g_loc, projectileList):
        ...
        if self.cannon.cooldown <= 0:
            ...
            if self.cannon.canFire == True:
                self.cannon.shoot(projectileList)
...
class SpaceGun(pygame.sprite.Sprite):#Actually moving part of gun
    ...
    def approachTurn(self, angle):#Find fastest path to angle, something's off with the final rotations
        ...
        #pygame.draw.circle(screen, (0,0,150), (self.tip), 5)
        #pygame.draw.circle(screen, (0,0,150), (self.portA), 5)
        #pygame.draw.circle(screen, (0,0,150), (self.portB), 5)

    def shoot(self, projectileList):
        projectileList.add(pjt.Bullet(self.portA[0], self.portA[1], "Cannon", 10, self.angle, 20))#I need to move the cannon's ports to a better location
        projectileList.add(pjt.Bullet(self.portB[0], self.portB[1], "Cannon", 10, self.angle, 20))#I need to move the cannon's ports to a better location 
